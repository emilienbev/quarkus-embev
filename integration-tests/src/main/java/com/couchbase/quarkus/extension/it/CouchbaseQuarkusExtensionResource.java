...
public class CouchbaseQuarkusExtensionResource {
    ...
    public String checkOtel() {
        ...
        Cluster cluster = Cluster.connect("couchbase://127.0.0.1", ClusterOptions.clusterOptions("Administrator", "password")
                .environment(env -> {
                    // Provide the OpenTelemetry object to the Couchbase SDK
                    env.requestTracer(OpenTelemetryRequestTracer.wrap(openTelemetry));
                }));

        cluster.query("select 1 as 1");
